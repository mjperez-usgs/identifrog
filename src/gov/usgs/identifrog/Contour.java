...
public class Contour {
	...
	/*
	 * byte[] makeChainCode8() { int m = points.size(); if (m>1){ int[] xPoints = new int[m]; int[]
	 * yPoints = new int[m]; int k = 0; Iterator<Point> itr = points.iterator(); while
	 * (itr.hasNext() && k < m) { Point cn = itr.next(); xPoints[k] = cn.x; yPoints[k] = cn.y; k = k
	 * + 1; } return null; } else { // use circles for isolated pixels //Point cn = points.get(0);
	 * return null; } }
	 */
